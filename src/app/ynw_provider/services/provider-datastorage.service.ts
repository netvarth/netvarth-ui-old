...
export class ProviderDataStorageService {
  ...
  updateGalleryWeightageToBusinessProfile(data) {
    const galleryObject = projectConstantsLocal.BUSINESS_PROFILE_WEIGHTAGE.MEDIA_GALLERY;
...
updateLanguagesWeightage(data) {
  const languageObject = projectConstantsLocal.BUSINESS_PROFILE_WEIGHTAGE.LANGUAGES_KNOWN;
...
updateSpecilizationWeightage(data) {
  const specializationObject = projectConstantsLocal.BUSINESS_PROFILE_WEIGHTAGE.SPECIALIZATION;
...
updateProfilePicWeightage(isImageExist) {
  const profilePicObject = projectConstantsLocal.BUSINESS_PROFILE_WEIGHTAGE.PROFILE_PIC;
