...
@Component({
  ...
  templateUrl: './mainservice-detail.component.html',
  styleUrls: ['./mainservice-detail.component.css']
})
...
export class MainserviceDetailComponent implements OnInit {
 ...
 // service;
  ...
  constructor(
    public dialogRef: MatDialogRef<MainserviceDetailComponent>,
  ...
  ngOnInit() {
    ...
    this.activaterouterobj.queryParams.subscribe(qparams => {
     console.log(qparams);
    // this.minutes = this.service.serviceDuration
     if (qparams.bname) {
      this.busname = qparams.bname;
    }
    if (qparams.sector) {
      this.sector = qparams.sector;
    }
     if (qparams.serv_type) {
