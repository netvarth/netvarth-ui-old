...
export class SearchDetailComponent implements OnInit, OnDestroy {
          ...
          this.search_return = this.shared_service.DocloudSearch(url, projectConstants.searchpass_criteria, this.kwtyp)
            .subscribe(res => {
              ...
              for (let i = 0; i < this.search_data.hits.hit.length; i++) {
                ...
                if (this.search_data.hits.hit[i].fields.account_type === '0' && this.search_data.hits.hit[i].fields.providers && this.search_data.hits.hit[i].fields.providers.length > 0) {
                  ...
                  } else if (this.search_data.hits.hit[i].fields.providers.length > 1) {
  ...
  serviceClicked(name, obj, origin) {
    ...
    if (origin === 'donationList' || origin === 'serviceListClick' || origin === 'appointmentList') {
      ...
      //     res => {
      // const s3url = res;
      this.s3Processor.getPresignedUrls(s3id,
        null, 'services').subscribe(
          (accountS3s) => {
            let servicesList: any = [];
            servicesList = accountS3s['services'];
  ...
  openCoupons(obj, type?) {
    ...
    this.s3Processor.getPresignedUrls(s3id,
      null, 'coupon,providerCoupon').subscribe(
        (accountS3s) => {
          couponObject.JC = accountS3s['coupon'];
          couponObject.OWN = accountS3s['providerCoupon'];
      ...
      null, 'jaldeediscount').subscribe(
        (accountS3s) => {
          this.jdndialogRef = this.dialog.open(JdnComponent, {
            ...
            disableClose: true,
            data: {
              jdnList: accountS3s['jaldeediscount']
            }
          });
