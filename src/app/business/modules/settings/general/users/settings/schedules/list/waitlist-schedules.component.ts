...
export class WaitlistuserSchedulesComponent implements OnInit, OnDestroy {
    ...
    changeScheduleSameDayOnlineStatus(qObj) {
        ...
        this.provider_services.changeSamedayAppointmentStatus(qObj.id, !qObj.todayAppt)
            .subscribe(() => {
                this.snackbarService.openSnackBar('Same day online appointments ' + chstatusmsg + ' successfully');
                this.initializeQs();
            },
                error => {
                    this.initializeQs();
    ...
    changeScheduleFutureStatus(qObj) {
        ...
        this.provider_services.changeFutureAppointmentStatus(qObj.id, !qObj.futureAppt)
            .subscribe(() => {
                this.snackbarService.openSnackBar('Future Appointment ' + chstatusmsg + ' successfully');
                this.initializeQs();
            },
                error => {
                    this.initializeQs();
    ...
    getTomorrowAppointmentCount(queue, index, origin) {
      ...
      const server = server_date.toLocaleString(this.dateTimeProcessor.REGION_LANGUAGE, { timeZone: this.dateTimeProcessor.TIME_ZONE_REGION });
