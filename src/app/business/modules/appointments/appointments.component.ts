...
export class AppointmentsComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    initView(view) {
      return new Promise((resolve, reject) => {
        ...
        this.selQIds = view.customViewConditions.queues[0]['id'];
...
if (this.selQIds) {
  this.getAvaiableSlots();
    ...
    getAvaiableSlots(type?) {
      this.provider_services.getAppointmentSlotsByScheduleid(this.selQIds).subscribe(data => {
        this.availableSlots = data;
        console.log(this.availableSlots.slot);
        Object.keys(this.availableSlots.slot).forEach(key => {
console.log(key);
console.log(this.availableSlots.slot[key]);
        });
      });
