...
@Component({
  selector: 'app-rx-home',
  templateUrl: './rx-home.component.html',
  styleUrls: ['./rx-home.component.css']
})
export class RxHomeComponent implements OnInit {
  addDrugdialogRef;
  drugList: any = [];
  today = new Date();
  patientDetails;
...
addDrug() {
  ...
  this.addDrugdialogRef.afterClosed().subscribe(result => {
    if (result) {
      ...
      this.router.navigate(['/provider/medicalrecord/home/prescription/addrxlist'], navigationExtras);
    }
  });
}
getMrprescription() {
