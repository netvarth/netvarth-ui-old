...
export class PrescriptionComponent implements OnInit, OnChanges {
  ...
  print(divName, signatureInfo) {
    ...
    if (signatureInfo === undefined) {
      const error = 'Please create your digital Signature via share';
      ...
      setTimeout(() => {
       ...
       if (this.blogo) {
        ...
        if (this.bname) {
          checkin_html += '<div style="font-size:18px;font-weight:bold;padding-right:150px; margin-top:150px;">' + this.bname;
          checkin_html += '</div>'
        ...
        checkin_html += '</div>';
        if (this.address) {
          checkin_html += '<div style="font-size:15px;font-weight:bold;padding-right:150px; ">' + this.address;
          checkin_html += '</div>'
        ...
        checkin_html += '<div>';
        checkin_html += '<br>';
        checkin_html += '<div style="border-bottom : 1.02503px solid #E7E3E3">';
        ...
        if (this.tempMedicalRecord.providerConsumer.gender) {
          checkin_html += '<div style = "float : right;">' + 'Gender : ' + this.tempMedicalRecord.providerConsumer.gender;
          checkin_html += '</div>'
        }
        ...
        checkin_html += '<br>';
        checkin_html += '</div>';
        if (this.tempMedicalRecord.providerConsumer.age && this.tempMedicalRecord.providerConsumer.age.year && this.tempMedicalRecord.providerConsumer.age.month === 0) {
          checkin_html += '<div style = "float : right;">' + 'Age : ' + this.tempMedicalRecord.providerConsumer.age.year + ' years';
          checkin_html += '</div>'
        }
        if (this.tempMedicalRecord.providerConsumer.age && this.tempMedicalRecord.providerConsumer.age.month && this.tempMedicalRecord.providerConsumer.age.year === 0) {
          checkin_html += '<div style = "float : right;">' + 'Age : ' + this.tempMedicalRecord.providerConsumer.age.month + ' months';
          checkin_html += '</div>'
        }
        ...
        checkin_html += '<br>';
        checkin_html += '</div>';
        checkin_html += '<div style = "float : right;">' + 'Date : ' + this.tempMedicalRecord.mrCreatedDate;
        checkin_html += '</div>'
        checkin_html += '<div>';
        checkin_html += '<br>';
        checkin_html += '</div>';
        ...
        for (let i = 0; i < _this.drugList.length; i++) {
          ...
          checkin_html += '<td style="padding:10px;">' + _this.drugList[i].medicine_name + '</td>';
          checkin_html += '<td style="padding:10px;">' + _this.drugList[i].duration + '</td>';
          checkin_html += '<td style="padding:10px;">' + _this.drugList[i].dosage + '</td>';
          checkin_html += '<td style="padding:10px;">' + _this.drugList[i].frequency + '</td>';
          checkin_html += '<td style="padding:10px;">' + _this.drugList[i].instructions + '</td>';
          checkin_html += '</tr>';
        }
        checkin_html += '</table>';
        checkin_html += '<div>';
      ...
      checkin_html += '<br>';
      checkin_html += '</div>';
      checkin_html += '<div style="font-size:14px;font-weight:bold;padding-right:150px; margin-top:200px"> Signature</div>'
      checkin_html += '<div>';
      checkin_html += '<br>';
      checkin_html += '</div>';
    ...
    if (url.fileType === '.pdf') {
      ...
      if (this.small_device_display === true) {
        ...
        if (url && url.s3path) {
          ...
          checkin_html += '<div style="display:flex;margin-bottom:10px;align-items:center;gap:5px"><div><img style="height:60px;width:60px;" src="/assets/images/medicalReportIcon/mr.webp" /></div><div style="font-size:14px;font-weight:bold">Prescription Invoice</div></div>'
          checkin_html += '<img style="width:100%;height:100%" src="' + url.s3path + '" />'
          checkin_html += '</div>'
          printWindow.document.write('<html><head><title></title>');
          printWindow.document.write('</head><body >');
          printWindow.document.write(checkin_html);
  ...
  saveAndAddOther(form_data) {
    ...
    this.ins = form_data.instructions;
    this.dur = form_data.duration;
    // this.api_loading=true;
  ...
  deletePrescriptionFile(fileDetails, index) {
    ...
    this.filesToUpload.push(fileDetails);
    this.deleteTempImagePrescription(this.filesToUpload, index)
  }


  deleteTempImagePrescription(filesToUpload, index) {
    // this.showSave = true;
    ...
    removeprescriptiondialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.provider_services.deleteUplodedprescriptionNew(this.mrId, filesToUpload)
          .subscribe((data) => {
            // console.log('data',data)
            this.selectedMessage.files.splice(index, 1);
